...
import com.danielasfregola.tutorial.cat.applicative._
...
object MonadInstances {
  ...
  implicit val maybeMonad: Monad[Maybe] = new Monad[Maybe] {
    ...
    def pure[A](a: A): Maybe[A] = ApplicativeInstances.maybeApplicative.pure(a)
  }
  ...
  implicit val zeroOrMoreMonad: Monad[ZeroOrMore] = new Monad[ZeroOrMore] {
    override def flatMap[A, B](boxA: ZeroOrMore[A])(f: (A) => ZeroOrMore[B]): ZeroOrMore[B] = boxA match {
      case OneOrMore(a, tail) => f(a).append(flatMap(tail)(f))
      case Zero => Zero
    }

    def pure[A](a: A): ZeroOrMore[A] = ApplicativeInstances.zeroOrMoreApplicative.pure(a)
  }
}
